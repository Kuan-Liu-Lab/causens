...
whether all confounding variables are contained in $X$. Hereon, we define $U$, the set of
unmeasured confounders, although, in `causens`, we assume $U$ to be univariate for simplicity.
...
effect by then marginalizing over $U$. For instance, this is the default mathematical model posited in `causens`:

\begin{align*}
  U &\sim \operatorname{Bern}(0.5) \\
  Z \mid U, X &\sim \text{Bern}(\text{logit}^{-1}(\alpha_{uz} U + \alpha_{xz} X)) \\
